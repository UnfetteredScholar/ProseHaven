...
@router.get(path="/stories/{story_id}/chapters", response_model=List[Dict[str, Any]])
def get_chapters(
    story_id: str, limit: int = 0, with_content: bool = False
) -> List[Dict[str, Any]]:
...
@router.get(
    path="/stories/{story_id}/chapters/{chapter_id}", response_model=Dict[str, Any]
)
def get_chapter(story_id: str, chapter_id: str) -> Dict[str, Any]:
...
@router.post("/stories/{story_id}/chapters", response_model=Dict[str, str])
def create_chapter(
    data: p.ChapterIn,
    story_id: str,
    ...
    try:
        ...
        message = {"mesage": "Chapter created successfully", "id": id}
...
@router.patch(
    "/stories/{story_id}/chapters/{chapter_id}", response_model=Dict[str, str]
)
def update_chapter_details(
    story_id: str,
    chapter_id: str,
    data: p.ChapterUpdate,
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    ...
    message = {"mesage": "Chapter updated successfully", "id": chapter_id}
...
def delete_chapter(
    ...
    storage.delete_chapter_record(story_id, chapter_id, current_user["_id"])

    message = {"mesage": "Chapter deleted successfully", "id": chapter_id}
