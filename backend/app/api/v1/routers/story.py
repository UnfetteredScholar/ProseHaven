...
@router.get(path="/stories", response_model=List[Dict[str, Any]])
...
@router.get(path="/stories/{story_id}", response_model=Dict[str, Any])
...
def create_story(
    data: p.StoryIn, current_user: Dict[str, Any] = Depends(get_current_user)
):
    ...
    try:
        ...
        message = {"mesage": "Story created successfully", "id": id}
...
@router.patch("/stories/{story_id}", response_model=Dict[str, str])
def update_story_details(
    ...
    message = {"mesage": "Story updated successfully", "id": story_id}

    return JSONResponse(message)


@router.delete("/stories/{story_id}", response_model=Dict[str, str])
def delete_story(
    story_id: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
    """Deletes a story under a user"""

    storage.delete_story_record(story_id, current_user["_id"])

    message = {"mesage": "Story deleted successfully", "id": story_id}
